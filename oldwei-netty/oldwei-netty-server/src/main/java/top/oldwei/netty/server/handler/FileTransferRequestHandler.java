...
public class FileTransferRequestHandler extends SimpleChannelInboundHandler<FileTransferRequestPacket> {
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, FileTransferRequestPacket fileTransferRequestPacket) throws Exception {
        log.debug("--------> {}", JSONObject.toJSONString(fileTransferRequestPacket));
        ...
        if(fileInfoCache.containsKey(fileTransferRequestPacket.getMd5())){
            fileInfoCache.get(fileTransferRequestPacket.getMd5()).setTransferPos(fileTransferRequestPacket.getEndPos());
            fileInfoCache.get(fileTransferRequestPacket.getMd5()).setLastModified(file.lastModified());
        }
